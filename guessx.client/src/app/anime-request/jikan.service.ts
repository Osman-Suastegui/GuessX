...
export class JikanService {
  ...
  getAnimeList(query: { q?: string; page?: number; limit?: number; type?: string; status?: string }): Observable<AnimeSearchResponse> {
  ...
  getAnimePictures(id: number): Observable<{ url: string; imageType: string; id: number }[]> {
    ...
    return forkJoin(requests).pipe(map((resultsArrays) => resultsArrays.flat()));
  }
}
